...
class Classroom(QWidget):
    def __init__(self, classroom, window):
        super().__init__()
        self.classroom = classroom
        self.w = window
        self.init_ui()
    ...
    def init_ui(self):
        ...
        self.setGeometry(100, 50, 825, 900)
        ...
        self.desks = []
        for col in range(3):  # 3 вертикальных ряда
            for row in range(6):  # 6 парт в каждом ряду
    ...
    def arrange_students(self):
        ...
        for desk in self.desks:
            desk.slot1.setText("")
            desk.slot2.setText("")
        for desk_id in range(6):
            desk = self.desks[desk_id]
            desk.slot1.setText(str(self.classroom[0][desk_id][0][1]))
            desk.slot2.setText(str(self.classroom[0][desk_id][1][1]))

        for desk_id in range(6, 12):
            desk = self.desks[desk_id]
            desk.slot1.setText(str(self.classroom[1][desk_id - 6][0][1]))
            desk.slot2.setText(str(self.classroom[1][desk_id - 6][1][1]))

        for desk_id in range(12, 18):
            desk = self.desks[desk_id]
            desk.slot1.setText(str(self.classroom[2][desk_id - 12][0][1]))
            desk.slot2.setText(str(self.classroom[2][desk_id - 12][1][1]))